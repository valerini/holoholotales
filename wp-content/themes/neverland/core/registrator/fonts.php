...
function get_fonts_array_only_key_name()
{
    ...
    global $gt3_fonts;
    $gt3_fonts_decoded = json_decode($gt3_fonts);

    if (is_array($gt3_fonts_decoded->items)) {
        foreach ($gt3_fonts_decoded->items as $key => $font) {
...
function get_theme_fonts()
{
    ...
    $allFonts[] = get_theme_mod('main_font', $GLOBALS["gt3_default_font"]) . ":300,400,700";
